...
typedef enum __NRFS_PACKED {
	NRFS_SWEXT_REQ_RESPONSE,	/** Request response. */
	NRFS_SWEXT_REQ_REJECTED,	/** Request rejected. */
} nrfs_swext_evt_type_t;

/** @brief SWEXT control service response notification types. */
typedef enum __NRFS_PACKED {
	NRFS_SWEXT_OUTPUT_ENABLED,	/** SWEXT powered up correctly. */
	NRFS_SWEXT_OUTPUT_DISABLED,	/** SWEXT powered down correctly. */
	NRFS_SWEXT_OVERCURRENT,		/** Overcurrent detected. SWEXT is powered down */
} nrfs_swext_evt_status_t;

/** @brief SWEXT control service response data structure. */
typedef struct {
	nrfs_swext_evt_type_t type;
	nrfs_swext_evt_status_t status;
} nrfs_swext_evt_t;

/** @brief SWEXT control service event handler type. */
typedef void (*nrfs_swext_evt_handler_t)(nrfs_swext_evt_t const *p_evt,
						void *context);
...
static inline uint8_t nrfs_load_current_to_raw(uint16_t load_current)
{
