...
typedef enum __NRFS_PACKED {
	...
	NRFS_GDFS_EVT_FREQ_CONFIRMED, /** Operating frequency has been acheived. */
} nrfs_gdfs_evt_type_t;

/** @brief Dynamic Voltage and Frequency Scaling service event. */
typedef struct {
	nrfs_gdfs_evt_type_t type; /** Event type. */
	enum gdfs_frequency_setting freq;		   /** Maximum allowed HSFLL frequency oppoint. */
} nrfs_gdfs_evt_t;
 ...
 * until the NRFS_GDFS_EVT_OPPOINT_REQ_CONFIRMED response is triggered.
 *
...
/**
 * @brief Function for requesting an operating frequency change.
 * @note The response @p NRFS_GDFS_EVT_OPPOINT_REQ_CONFIRMED will not be triggered.
 *
 * @param[in] target_freq  Minimal required frequency oppoint
 * @param[in] p_context    Opaque user data that will be passed to registered callback.
 *
