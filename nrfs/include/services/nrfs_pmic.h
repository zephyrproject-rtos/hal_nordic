...
/**
 * @brief Function for subscribing to POFWARN notification
 *
 * Sets the power-fail warning threshold voltage and subscribes
 * the caller to the POFWARN notification. Each subscriber
 * maintains its own threshold. Set @p volts to 0 to unsubscribe.
 *
 * @param[in] volts      Threshold voltage value (decivolts). 0 to unsubscribe.
...
/**
 * @brief Function for reading POFWARN status for this subscriber
 *
 * Returns the current POFWARN comparator status relative to the subscriber's
 * own threshold. The subscriber must have set a threshold via
 * @ref nrfs_pmic_pofwarn_th_set before calling this function.
 *
 * @param[in] p_context  Opaque user data that will be passed
 *                        to registered callback.
 *
