 ...
 *       The function returns NRF_LPCOMP_INPUT_DISABLED for analog inputs that are not
...
NRFX_STATIC_INLINE nrf_lpcomp_input_t nrfx_lpcomp_ain_get(nrfx_analog_input_t input)
{
    ...
    return NRF_LPCOMP_INPUT_DISABLED;
}
 ...
 *
 * @note The function takes into account the internal and external analog inputs.
 *       The function returns NRF_LPCOMP_EXT_REF_DISABLED for reference inputs that are not
 *       supported by the given device.
 */
NRFX_STATIC_INLINE nrf_lpcomp_ext_ref_t nrfx_lpcomp_ext_ref_get(nrfx_analog_input_t input)
{
    ...
    return NRF_LPCOMP_EXT_REF_DISABLED;
}
