...
nrf_wifi_fmac_fw_init_rt(struct nrf_wifi_fmac_dev_ctx* fmac_dev_ctx,
                         struct nrf_wifi_phy_rf_params* rf_params, bool rf_params_valid,
                         ...
                         struct nrf_wifi_board_params* board_params, unsigned char* country_code)
{
    ...
    if (!fmac_dev_ctx)
    {
        nrf_wifi_osal_log_err("%s: Invalid device context", __func__);
        goto out;
    }

    status =
        umac_cmd_init(fmac_dev_ctx, rf_params, rf_params_valid,
#ifdef NRF_WIFI_LOW_POWER
                      sleep_type,
#endif /* NRF_WIFI_LOW_POWER */
    ...
    {
        nrf_wifi_osal_log_err("%s: UMAC init failed", __func__);
        goto out;
    }
                          ...
                          struct nrf_wifi_board_params* board_params, unsigned char* country_code)
{
    ...
    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: nrf_wifi_hal_dev_init failed", __func__);
        goto out;
    }

    fmac_dev_ctx->tx_pwr_ceil_params = nrf_wifi_osal_mem_alloc(sizeof(*tx_pwr_ceil_params));
    nrf_wifi_osal_mem_cpy(fmac_dev_ctx->tx_pwr_ceil_params, tx_pwr_ceil_params,
                          sizeof(*tx_pwr_ceil_params));

    nrf_wifi_osal_mem_set(&otp_info, 0xFF, sizeof(otp_info));

    status = nrf_wifi_hal_otp_info_get(fmac_dev_ctx->hal_dev_ctx, &otp_info.info, &otp_info.flags);

    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: Fetching of RPU OTP information failed", __func__);
        goto out;
    }

    status = nrf_wifi_fmac_rf_params_get(fmac_dev_ctx, &phy_rf_params);

    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: RF parameters get failed", __func__);
        goto out;
    }

    status = nrf_wifi_fmac_fw_init_rt(fmac_dev_ctx, &phy_rf_params, true,
#ifdef NRF_WIFI_LOW_POWER
                                      sleep_type,
#endif /* NRF_WIFI_LOW_POWER */
    ...
    {
        nrf_wifi_osal_log_err("%s: nrf_wifi_fmac_fw_init failed", __func__);
        goto out;
    }
out:
    return status;
...
struct nrf_wifi_fmac_priv* nrf_wifi_fmac_init_rt(void)
{
    ...
    if (!fpriv)
    {
        nrf_wifi_osal_log_err("%s: Unable to allocate fpriv", __func__);
        goto out;
    }

    nrf_wifi_osal_mem_set(&hal_cfg_params, 0, sizeof(hal_cfg_params));

    hal_cfg_params.max_cmd_size = MAX_NRF_WIFI_UMAC_CMD_SIZE;
    hal_cfg_params.max_event_size = MAX_EVENT_POOL_LEN;

    fpriv->hpriv = nrf_wifi_hal_init(&hal_cfg_params, &nrf_wifi_fmac_event_callback, NULL);

    if (!fpriv->hpriv)
    {
        nrf_wifi_osal_log_err("%s: Unable to do HAL init", __func__);
                                               ...
                                               unsigned int timeout)
{
    ...
    do
    {
        nrf_wifi_osal_sleep_ms(1);
        count++;
    } while ((!rt_dev_ctx->radio_cmd_done) && (count < timeout));

    if (count == timeout)
    {
        nrf_wifi_osal_log_err("%s: Timed out (%d secs)", __func__, timeout / 1000);
        goto out;
    }
    ...
    {
        nrf_wifi_osal_log_err("%s: Radio test command failed with status %d", __func__,
                              radio_cmd_status);
        goto out;
    }
                                                   ...
                                                   struct rpu_conf_params* params)
{
    ...
    nrf_wifi_osal_mem_set(&init_params, 0, sizeof(init_params));

    nrf_wifi_osal_mem_cpy(init_params.rf_params, params->rf_params, NRF_WIFI_RF_PARAMS_SIZE);

    nrf_wifi_osal_mem_cpy(&init_params.chan, &params->chan, sizeof(init_params.chan));

    init_params.phy_threshold = params->phy_threshold;
    init_params.phy_calib = params->phy_calib;

    rt_dev_ctx->radio_cmd_done = false;
    status = umac_cmd_prog_init(fmac_dev_ctx, &init_params);

    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: Unable to init radio test", __func__);
        goto out;
    }

    status = wait_for_radio_cmd_status(fmac_dev_ctx, RADIO_CMD_STATUS_TIMEOUT);
    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        goto out;
    }
                                                      ...
                                                      struct rpu_conf_params* params)
{
    ...
    rt_dev_ctx->radio_cmd_done = false;
    status = umac_cmd_prog_tx(fmac_dev_ctx, params);

    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: Unable to program radio test TX", __func__);
        goto out;
    }
                                                      ...
                                                      struct rpu_conf_params* params)
{
    ...
    nrf_wifi_osal_mem_set(&rx_params, 0, sizeof(rx_params));

    rx_params.nss = params->nss;

    nrf_wifi_osal_mem_cpy(rx_params.rf_params, params->rf_params, NRF_WIFI_RF_PARAMS_SIZE);

    nrf_wifi_osal_mem_cpy(&rx_params.chan, &params->chan, sizeof(rx_params.chan));
    ...
    rt_dev_ctx->radio_cmd_done = false;
    status = umac_cmd_prog_rx(fmac_dev_ctx, &rx_params);

    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: Unable to program radio test RX", __func__);
        goto out;
    }
...
enum nrf_wifi_status nrf_wifi_fmac_rf_test_rx_cap(struct nrf_wifi_fmac_dev_ctx* fmac_dev_ctx,
                                                  ...
                                                  void* cap_data, unsigned short int num_samples,
                                                  unsigned short int capture_timeout,
                                                  unsigned char lna_gain, unsigned char bb_gain,
                                                  unsigned char* capture_status)
{
    ...
    nrf_wifi_osal_mem_set(&rf_test_cap_params, 0, sizeof(rf_test_cap_params));
    ...
    status = umac_cmd_prog_rf_test(fmac_dev_ctx, &rf_test_cap_params, sizeof(rf_test_cap_params));

    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: umac_cmd_prog_rf_test_cap failed", __func__);
                                                   ...
                                                   signed char tx_power)
{
    ...
    nrf_wifi_osal_mem_set(&rf_test_tx_params, 0, sizeof(rf_test_tx_params));
    ...
    status = umac_cmd_prog_rf_test(fmac_dev_ctx, &rf_test_tx_params, sizeof(rf_test_tx_params));

    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: umac_cmd_prog_rf_test_tx_tone failed", __func__);
                                               ...
                                               unsigned char enable)
{
    ...
    nrf_wifi_osal_mem_set(&rf_test_dpd_params, 0, sizeof(rf_test_dpd_params));
    ...
    status = umac_cmd_prog_rf_test(fmac_dev_ctx, &rf_test_dpd_params, sizeof(rf_test_dpd_params));

    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: umac_cmd_prog_rf_test_dpd failed", __func__);
...
enum nrf_wifi_status nrf_wifi_fmac_rf_get_temp(struct nrf_wifi_fmac_dev_ctx* fmac_dev_ctx)
{
    ...
    nrf_wifi_osal_mem_set(&rf_test_get_temperature, 0, sizeof(rf_test_get_temperature));
    ...
    status = umac_cmd_prog_rf_test(fmac_dev_ctx, &rf_test_get_temperature,
                                   sizeof(rf_test_get_temperature));

    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: umac_cmd_prog_rf_get_temperature failed", __func__);
...
enum nrf_wifi_status nrf_wifi_fmac_rf_get_bat_volt(struct nrf_wifi_fmac_dev_ctx* fmac_dev_ctx)
{
    ...
    unsigned int count = 0;
    rt_dev_ctx = wifi_dev_priv(fmac_dev_ctx);
    nrf_wifi_osal_mem_set(fmac_dev_ctx->fpriv->opriv, &rf_test_get_temperature, 0,
                          sizeof(rf_test_get_temperature));
    ...
    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err(fmac_dev_ctx->fpriv->opriv,
                              "%s: umac_cmd_prog_rf_get_temperature failed", __func__);
        goto out;
    }
    do
    {
...
enum nrf_wifi_status nrf_wifi_fmac_rf_get_rf_rssi(struct nrf_wifi_fmac_dev_ctx* fmac_dev_ctx)
{
    ...
    nrf_wifi_osal_mem_set(&rf_get_rf_rssi_params, 0, sizeof(rf_get_rf_rssi_params));
    ...
    status =
        umac_cmd_prog_rf_test(fmac_dev_ctx, &rf_get_rf_rssi_params, sizeof(rf_get_rf_rssi_params));

    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: umac_cmd_prog_rf_get_rf_rssi failed", __func__);
                                              ...
                                              unsigned char value)
{
    ...
    nrf_wifi_osal_mem_set(&nrf_wifi_rf_test_xo_calib_params, 0,
    ...
    status = umac_cmd_prog_rf_test(fmac_dev_ctx, &nrf_wifi_rf_test_xo_calib_params,
                                   sizeof(nrf_wifi_rf_test_xo_calib_params));

    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: umac_cmd_prog_set_xo_val failed", __func__);
...
enum nrf_wifi_status nrf_wifi_fmac_rf_test_compute_xo(struct nrf_wifi_fmac_dev_ctx* fmac_dev_ctx)
{
    ...
    nrf_wifi_osal_mem_set(&rf_get_xo_value_params, 0, sizeof(rf_get_xo_value_params));
    ...
    status = umac_cmd_prog_rf_test(fmac_dev_ctx, &rf_get_xo_value_params,
                                   sizeof(rf_get_xo_value_params));

    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: umac_cmd_prog_rf_get_xo_value failed", __func__);
