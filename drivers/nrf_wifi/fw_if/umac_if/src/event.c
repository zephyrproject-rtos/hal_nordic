                                       ...
                                       enum nrf_wifi_fmac_if_carr_state carr_state)
{
    ...
    {
        nrf_wifi_osal_log_dbg("%s: No callback handler registered", __func__);
    ...
    {
        nrf_wifi_osal_log_err("%s: Invalid wdev_id recd from UMAC %d", __func__, if_idx);
        goto out;
    }

    vif_ctx = def_dev_ctx->vif_ctx[if_idx];

    status = def_priv->callbk_fns.if_carr_state_chg_callbk_fn(vif_ctx->os_vif_ctx, carr_state);

    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: IF carrier state change failed for VIF idx = %d", __func__,
                              if_idx);
        goto out;
    }
out:
...
static void umac_event_connect(struct nrf_wifi_fmac_dev_ctx* fmac_dev_ctx, void* event_data)
{
    ...
    {
        nrf_wifi_osal_log_err("%s: Invalid wdev_id recd from UMAC %d", __func__, if_index);
        return;
    }
    ...
    if (event->umac_hdr.cmd_evnt == NRF_WIFI_UMAC_EVENT_NEW_STATION)
    {
        ...
        {
            nrf_wifi_osal_mem_cpy(vif_ctx->bssid, event->mac_addr, NRF_WIFI_ETH_ADDR_LEN);
        }
        peer_id = nrf_wifi_fmac_peer_get_id(fmac_dev_ctx, event->mac_addr);

        if (peer_id == -1)
        {
            peer_id = nrf_wifi_fmac_peer_add(fmac_dev_ctx, if_index, event->mac_addr,
                                             event->is_sta_legacy, event->wme);

            if (peer_id == -1)
            {
                nrf_wifi_osal_log_err("%s:Can't add new station.", __func__);
                return;
            }
        }
    }
    else if (event->umac_hdr.cmd_evnt == NRF_WIFI_UMAC_EVENT_DEL_STATION)
    {
        ...
        if (peer_id != -1)
        {
            nrf_wifi_fmac_peer_remove(fmac_dev_ctx, if_index, peer_id);
        }
    }
                                                    ...
                                                    void* event_data, unsigned int event_len)
{
    ...
    {
        nrf_wifi_osal_log_err("%s: Invalid wdev_id recd from UMAC %d", __func__, if_id);
...
#else
    nrf_wifi_osal_log_dbg("%s: Event %d received from UMAC", __func__, event_num);
    ...
    case NRF_WIFI_UMAC_EVENT_GET_REG:
        ...
        if (callbk_fns->event_get_reg)
            callbk_fns->event_get_reg(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
#endif /* NRF70_STA_MODE */
#ifdef NRF70_RADIO_TEST
        get_reg_event = (struct nrf_wifi_reg*) event_data;

        nrf_wifi_osal_mem_cpy(&fmac_dev_ctx->alpha2, &get_reg_event->nrf_wifi_alpha2,
    ...
    case NRF_WIFI_UMAC_EVENT_REG_CHANGE:
        ...
        if (callbk_fns->reg_change_callbk_fn)
            callbk_fns->reg_change_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
#endif /* NRF70_STA_MODE */
        ...
        {
            nrf_wifi_osal_log_err("%s: Failed to allocate memory for reg_change", __func__);
            goto out;
        }

        nrf_wifi_osal_mem_cpy(fmac_dev_ctx->reg_change, reg_change_event,
    ...
    case NRF_WIFI_UMAC_EVENT_TRIGGER_SCAN_START:
        if (callbk_fns->scan_start_callbk_fn)
            callbk_fns->scan_start_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_SCAN_DONE:
        if (callbk_fns->scan_done_callbk_fn)
            callbk_fns->scan_done_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_SCAN_ABORTED:
        if (callbk_fns->scan_abort_callbk_fn)
            callbk_fns->scan_abort_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_SCAN_DISPLAY_RESULT:
        ...
        if (callbk_fns->disp_scan_res_callbk_fn)
            callbk_fns->disp_scan_res_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len,
                                                more_res);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
    ...
    case NRF_WIFI_UMAC_EVENT_IFFLAGS_STATUS:
        ...
        {
            nrf_wifi_osal_log_err("%s: Failed to set interface flags: %d", __func__,
    ...
    case NRF_WIFI_UMAC_EVENT_TWT_SLEEP:
        if (callbk_fns->twt_sleep_callbk_fn)
            callbk_fns->twt_sleep_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_SCAN_RESULT:
        ...
        if (callbk_fns->scan_res_callbk_fn)
            callbk_fns->scan_res_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len, more_res);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_AUTHENTICATE:
        if (callbk_fns->auth_resp_callbk_fn)
            callbk_fns->auth_resp_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_ASSOCIATE:
        if (callbk_fns->assoc_resp_callbk_fn)
            callbk_fns->assoc_resp_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_DEAUTHENTICATE:
        if (callbk_fns->deauth_callbk_fn)
            callbk_fns->deauth_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_DISASSOCIATE:
        if (callbk_fns->disassoc_callbk_fn)
            callbk_fns->disassoc_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_FRAME:
        if (callbk_fns->mgmt_rx_callbk_fn)
            callbk_fns->mgmt_rx_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_GET_TX_POWER:
        if (callbk_fns->tx_pwr_get_callbk_fn)
            callbk_fns->tx_pwr_get_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_GET_CHANNEL:
        if (callbk_fns->chnl_get_callbk_fn)
            callbk_fns->chnl_get_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_GET_STATION:
        if (callbk_fns->get_station_callbk_fn)
            callbk_fns->get_station_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_NEW_INTERFACE:
        if (callbk_fns->get_interface_callbk_fn)
            callbk_fns->get_interface_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_COOKIE_RESP:
        if (callbk_fns->cookie_rsp_callbk_fn)
            callbk_fns->cookie_rsp_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_FRAME_TX_STATUS:
        if (callbk_fns->mgmt_tx_status)
            callbk_fns->mgmt_tx_status(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    ...
    case NRF_WIFI_UMAC_EVENT_UNPROT_DISASSOCIATE:
        if (callbk_fns->unprot_mlme_mgmt_rx_callbk_fn)
            callbk_fns->unprot_mlme_mgmt_rx_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_SET_INTERFACE:
        if (callbk_fns->set_if_callbk_fn)
            callbk_fns->set_if_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_CONFIG_TWT:
        if (callbk_fns->twt_config_callbk_fn)
            callbk_fns->twt_config_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_TEARDOWN_TWT:
        if (callbk_fns->twt_teardown_callbk_fn)
            callbk_fns->twt_teardown_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_NEW_WIPHY:
        if (callbk_fns->event_get_wiphy)
            callbk_fns->event_get_wiphy(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
...
#endif
        nrf_wifi_osal_log_dbg("%s: Command %d -> status %d", __func__, cmd_status->cmd_id,
    ...
    case NRF_WIFI_UMAC_EVENT_GET_POWER_SAVE_INFO:
        if (callbk_fns->event_get_ps_info)
            callbk_fns->event_get_ps_info(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    ...
    case NRF_WIFI_UMAC_EVENT_DEL_STATION:
        umac_event_connect(fmac_dev_ctx, event_data);
        break;
#endif /* NRF70_STA_MODE */
#ifdef NRF70_P2P_MODE
    case NRF_WIFI_UMAC_EVENT_REMAIN_ON_CHANNEL:
        if (callbk_fns->roc_callbk_fn)
            callbk_fns->roc_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    case NRF_WIFI_UMAC_EVENT_CANCEL_REMAIN_ON_CHANNEL:
        if (callbk_fns->roc_cancel_callbk_fn)
            callbk_fns->roc_cancel_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
    ...
    case NRF_WIFI_UMAC_EVENT_GET_CONNECTION_INFO:
        if (callbk_fns->get_conn_info_callbk_fn)
            callbk_fns->get_conn_info_callbk_fn(vif_ctx->os_vif_ctx, event_data, event_len);
        else
            nrf_wifi_osal_log_err("%s: No callback registered for event %d", __func__,
                                  umac_hdr->cmd_evnt);
        break;
...
#endif /* !NRF70_RADIO_TEST && !NRF70_OFFLOADED_RAW_TX */
    default:
        nrf_wifi_osal_log_dbg("%s: No callback registered for event %d", __func__,
                              umac_hdr->cmd_evnt);
        break;
    }

    nrf_wifi_osal_log_dbg("%s: Event %d processed", __func__, event_num);
...
#if !defined(NRF70_RADIO_TEST) && !defined(NRF70_OFFLOADED_RAW_TX)
static enum nrf_wifi_status
nrf_wifi_fmac_data_event_process(struct nrf_wifi_fmac_dev_ctx* fmac_dev_ctx, void* umac_head)
{
    enum nrf_wifi_status status = NRF_WIFI_STATUS_SUCCESS;
    int event = -1;
    struct nrf_wifi_fmac_dev_ctx_def* def_dev_ctx = NULL;
    ...
    nrf_wifi_osal_log_info("%s: Event %d received from UMAC\n", __func__, event);
#else
    nrf_wifi_osal_log_dbg("%s: Event %d received from UMAC", __func__, event);
#endif /* CONFIG_NRF_WIFI_CMD_EVENT_LOG */
    ...
    case NRF_WIFI_CMD_RX_BUFF:
        ...
        struct nrf_wifi_rx_buff* config = nrf_wifi_osal_mem_zalloc(sizeof(struct nrf_wifi_rx_buff));
        if (!config)
        {
            nrf_wifi_osal_log_err("%s: Failed to allocate memory (RX)", __func__);
            status = NRF_WIFI_STATUS_FAIL;
            break;
        }
        nrf_wifi_osal_mem_cpy(config, umac_head, sizeof(struct nrf_wifi_rx_buff));
        status = nrf_wifi_utils_q_enqueue(def_dev_ctx->rx_config.rx_tasklet_event_q, config);
        if (status != NRF_WIFI_STATUS_SUCCESS)
        {
            nrf_wifi_osal_log_err("%s: Failed to enqueue RX buffer", __func__);
            nrf_wifi_osal_mem_free(config);
            break;
        }
        nrf_wifi_osal_tasklet_schedule(def_dev_ctx->rx_tasklet);
#else
        status = nrf_wifi_fmac_rx_event_process(fmac_dev_ctx, umac_head);
    ...
    case NRF_WIFI_CMD_TX_BUFF_DONE:
        ...
        struct nrf_wifi_tx_buff_done* config =
            nrf_wifi_osal_mem_zalloc(sizeof(struct nrf_wifi_tx_buff_done));
        if (!config)
        {
            nrf_wifi_osal_log_err("%s: Failed to allocate memory (TX)", __func__);
            status = NRF_WIFI_STATUS_FAIL;
            break;
        }
        nrf_wifi_osal_mem_cpy(config, umac_head, sizeof(struct nrf_wifi_tx_buff_done));
        status = nrf_wifi_utils_q_enqueue(def_dev_ctx->tx_config.tx_done_tasklet_event_q, config);
        if (status != NRF_WIFI_STATUS_SUCCESS)
        {
            nrf_wifi_osal_log_err("%s: Failed to enqueue TX buffer", __func__);
            nrf_wifi_osal_mem_free(config);
            break;
        }
        nrf_wifi_osal_tasklet_schedule(def_dev_ctx->tx_done_tasklet);
#else
        status = nrf_wifi_fmac_tx_done_event_process(fmac_dev_ctx, umac_head);
#endif /* NRF70_TX_DONE_WQ_ENABLED */
        break;
    case NRF_WIFI_CMD_CARRIER_ON:
        status = nrf_wifi_fmac_if_carr_state_event_proc(fmac_dev_ctx, umac_head,
...
out:
    if (status != NRF_WIFI_STATUS_SUCCESS)
    {
        nrf_wifi_osal_log_err("%s: Failed for event = %d", __func__, event);
    }
                                  ...
                                  struct host_rpu_msg* rpu_msg)
{
    ...
    {
        status = nrf_wifi_fmac_data_event_process(fmac_dev_ctx, umac_head);

        if (status != NRF_WIFI_STATUS_SUCCESS)
        {
            nrf_wifi_osal_log_err("%s: umac_process_data_event failed", __func__);
            goto out;
        }
                                                       ...
                                                       void* event)
{
    ...
    {
        nrf_wifi_osal_log_err("%s: Invalid event type (%d) recd for RF test type (%d)", __func__,
                              rf_test_event->rf_test_info.rfevent[0], def_dev_ctx->rf_test_type);
        goto out;
    }
    ...
    case NRF_WIFI_RF_TEST_EVENT_RX_DYN_PKT_CAP:
        status = hal_rpu_mem_read(fmac_dev_ctx->hal_dev_ctx, def_dev_ctx->rf_test_cap_data,
                                  RPU_MEM_RF_TEST_CAP_BASE, def_dev_ctx->rf_test_cap_sz);
    ...
    case NRF_WIFI_RF_TEST_EVENT_RF_RSSI:
        ...
        nrf_wifi_osal_log_info("RF RSSI value is = %d", rf_get_rf_rssi.agc_status_val);
        break;
    case NRF_WIFI_RF_TEST_EVENT_GET_BAT_VOLT:
        ...
        {
            nrf_wifi_osal_log_err(fmac_dev_ctx->fpriv->opriv, "Volatge reading failed");
        }
        else
        {
            voltage = BAT_CONST_1 + (BAT_CONST_2 * bat_volt_params.voltage);
            nrf_wifi_osal_log_info(fmac_dev_ctx->fpriv->opriv, "The battery voltage is = %lf Volts",
                                   voltage);
        }
        break;
    case NRF_WIFI_RF_TEST_EVENT_XO_CALIB:
        ...
        nrf_wifi_osal_log_info("XO value configured is = %d", xo_calib_params.xo_val);
        break;
    case NRF_WIFI_RF_TEST_XO_TUNE:
        ...
        nrf_wifi_osal_log_info("Best XO value is = %d", rf_get_xo_value_params.xo_value);
        break;
    default:
        break;
    }
                                                     ...
                                                     void* event)
{
    ...
    {
        nrf_wifi_osal_log_err("%s: Stats recd when req was not sent!", __func__);
        goto out;
    }

    stats = ((struct nrf_wifi_umac_event_stats*) event);

    nrf_wifi_osal_mem_cpy(fmac_dev_ctx->fw_stats, &stats->fw, sizeof(*fmac_dev_ctx->fw_stats));
                                     ...
                                     struct nrf_wifi_event_raw_config_mode* mode_event)
{
    ...
    if (!mode_event->status)
    {
        ...
        if ((mode_event->op_mode & NRF_WIFI_STA_MODE) == NRF_WIFI_STA_MODE)
        {
            ...
            if ((mode_event->op_mode ^ (NRF_WIFI_PROMISCUOUS_MODE | NRF_WIFI_TX_INJECTION_MODE)) ==
                0)
            {
                vif->if_type = NRF_WIFI_STA_PROMISC_TX_INJECTOR;
                config->peers[MAX_PEERS].peer_id = MAX_PEERS;
                config->peers[MAX_PEERS].if_idx = if_idx;
                vif->txinjection_mode = true;
                vif->promisc_mode = true;
            }
#endif /* NRF70_RAW_DATA_TX && defined NRF70_PROMISC_DATA_RX */
#ifdef NRF70_RAW_DATA_TX
            if ((mode_event->op_mode ^ NRF_WIFI_TX_INJECTION_MODE) == 0)
            {
                config->peers[MAX_PEERS].peer_id = MAX_PEERS;
                config->peers[MAX_PEERS].if_idx = if_idx;
...
#endif /* NRF70_RAW_DATA_TX */
#ifdef NRF70_PROMISC_DATA_RX
            if ((mode_event->op_mode ^ NRF_WIFI_PROMISCUOUS_MODE) == 0)
            {
        ...
        if ((mode_event->op_mode & NRF_WIFI_MONITOR_MODE) == NRF_WIFI_MONITOR_MODE)
        {
    ...
    {
        nrf_wifi_osal_log_err("%s: Set mode failed!", __func__);
        status = NRF_WIFI_STATUS_FAIL;
    }
out:
    return status;
                                                    ...
                                                    struct host_rpu_msg* rpu_msg)
{
    ...
    case NRF_WIFI_EVENT_RF_TEST:
        status = umac_event_rf_test_process(fmac_dev_ctx, sys_head);
        break;
    ...
    case NRF_WIFI_EVENT_RAW_TX_DONE:
        status = nrf_wifi_fmac_rawtx_done_event_process(
            fmac_dev_ctx, (struct nrf_wifi_event_raw_tx_done*) sys_head);
        break;
#endif
#ifdef NRF70_SYSTEM_WITH_RAW_MODES
    case NRF_WIFI_EVENT_MODE_SET_DONE:
        status = nrf_wifi_fmac_if_mode_set_event_proc(
            fmac_dev_ctx, (struct nrf_wifi_event_raw_config_mode*) sys_head);
        break;
#endif
    ...
    case NRF_WIFI_EVENT_CHANNEL_SET_DONE:
        ...
        {
            def_dev_ctx->vif_ctx[channel_event->if_index]->channel = channel_event->chan_num;
        }
        status = NRF_WIFI_STATUS_SUCCESS;
        break;
#endif /* NRF70_RAW_DATA_TX */
    ...
    case NRF_WIFI_EVENT_FILTER_SET_DONE:
        ...
        {
            def_dev_ctx->vif_ctx[filter_event->if_index]->packet_filter = filter_event->filter;
        }
        status = NRF_WIFI_STATUS_SUCCESS;
        break;
#endif /* NRF70_RAW_DATA_RX || NRF70_PROMISC_DATA_RX */
    default:
        nrf_wifi_osal_log_err("%s: Unknown event recd: %d", __func__,
...
enum nrf_wifi_status nrf_wifi_fmac_event_callback(void* mac_dev_ctx, void* rpu_event_data,
                                                  unsigned int rpu_event_len)
{
...
#ifdef CONFIG_NRF_WIFI_CMD_EVENT_LOG
    nrf_wifi_osal_log_info("%s: Event type %d recd\n", __func__, rpu_msg->type);
#else
    nrf_wifi_osal_log_dbg("%s: Event type %d recd", __func__, rpu_msg->type);
#endif /* CONFIG_NRF_WIFI_CMD_EVENT_LOG */
    ...
    case NRF_WIFI_HOST_RPU_MSG_TYPE_DATA:
        status = nrf_wifi_fmac_data_events_process(fmac_dev_ctx, rpu_msg);
        break;
#endif /* !NRF70_RADIO_TEST && !NRF70_OFFLOADED_RAW_TX */
    case NRF_WIFI_HOST_RPU_MSG_TYPE_UMAC:
        ...
        if (status != NRF_WIFI_STATUS_SUCCESS)
        {
            nrf_wifi_osal_log_err("%s: umac_event_ctrl_process failed", __func__);
            goto out;
        }
        break;
