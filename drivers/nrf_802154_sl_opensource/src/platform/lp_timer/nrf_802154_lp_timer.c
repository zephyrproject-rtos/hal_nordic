...
/**
 * @file
 *   This file contains standalone implementation of the nRF 802.15.4 timer
 *   abstraction.
 *
...
/* Time that has passed between overflow events. On full RTC speed, it occurs
 * every 512 s.
 */
...
struct compare_channel_descriptor_s {
	/* Channel number */
	uint32_t channel;
	/* Interrupt mask */
	uint32_t int_mask;
...
enum compare_channel_t { LP_TIMER_CHANNEL, SYNC_CHANNEL, CHANNEL_CNT };

/* Descriptors of all used compare channels. */
static const struct compare_channel_descriptor_s cmp_ch[CHANNEL_CNT] = {
...
/** @brief
 * Non-blocking mutex for mutual write access to @ref offset_counter variable.
 *
...
/** @brief
 * Get RTC counter value and matching offset that represent the current time.
 *
 ...
 *
 *  This function returns current value of offset_counter variable. If OVERFLOW
 *  event is present while calling this function, it is handled within it.
 *
...
static uint32_t overflow_counter_get(void)
{
	...
	if (mutex_get()) {
		...
		if (nrf_rtc_event_check(NRF_802154_RTC_INSTANCE,
			...
			/* Result should be incremented. offset_counter will be
			 * incremented after mutex is released.
			 */
		} else {
			 ...
			 * assured that offset_counter was incremented for
			 * the second time, so we increment the result here.
			 */
		}
 ...
 *
 * @note This function takes into account possible overflow of first 32 bits
 *       in current time.
 *
				      ...
				      const uint64_t *p_now)
{
	...
	/* Check if 32 LSB of `now` overflowed between getting t0 and loading
	 * `now` value.
	 */
...
/**
 * @brief
 * Start one-shot timer that expires at specified time on desired channel.
 *
 * Start one-shot timer that will expire @p dt microseconds after @p t0 time on
 * channel @p channel.
 *
 ...
 * @param[in]  dt       Time of timer expiration as time elapsed from
 *                      @p t0 [us].
 * @param[in]  p_now    Pointer to data with the current time.
 */
static void timer_start_at(enum compare_channel_t channel, uint32_t t0,
			   uint32_t dt, const uint64_t *p_now)
{
...
static void rtc_irq_handler(void)
{
	...
	if (nrf_rtc_event_check(NRF_802154_RTC_INSTANCE,
				NRF_RTC_EVENT_OVERFLOW)) {
		/* Disable OVERFLOW interrupt to prevent lock-up in interrupt
		 * context while mutex is locked from lower priority context
		...
		/* Handle OVERFLOW event by reading current value of overflow
		 * counter.
		 */
